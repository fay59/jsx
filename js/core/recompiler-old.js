...
OldRecompiler.prototype.recompileFunction = function(memory, startAddress)
{
...
OldRecompiler.prototype.recompileOpcode = function(currentAddress, op)
{
	...
	var addressString = OldRecompiler.formatHex(currentAddress);
...
OldRecompiler.prototype.addLabel = function(label, opAddress)
{
	var labelString = OldRecompiler.formatHex(label);
	var opAddressString = OldRecompiler.formatHex(opAddress);
...
OldRecompiler.prototype.match = function(instruction)
{
...
OldRecompiler.prototype.nextInstruction = function()
{
	if (this.memory.translate)
	{
		var translated = this.memory.translate(this.address);
		if (translated.buffer == MemoryMap.unmapped)
			this.panic("accessing invalid memory address " + OldRecompiler.formatHex(this.address));
	}
...
OldRecompiler.prototype.panic = function(instruction, address)
{
	if (isFinite(instruction))
	{
		...
		throw new Error("No matching instruction for pattern " + binary + " at " + OldRecompiler.formatHex(address));
	}
	else
	{
...
OldRecompiler.functionPrelude = "var overflowChecked = 0;\n";

OldRecompiler.prototype.compile = function()
{
	var jsCode = OldRecompiler.functionPrelude;
	jsCode += "while (true) {\n";
	...
	for (var i = 0; i < keys.length; i++)
	{
		...
		{
			jsCode += "case 0x" + OldRecompiler.formatHex(address) + ":\n";
	...
	jsCode += "default: this.panic('unreferenced block 0x' + OldRecompiler.formatHex(pc), pc); break;\n";
	jsCode += "}\n}";
	
	var functionName = "." + OldRecompiler.formatHex(this.startAddress);
...
OldRecompiler.prototype.recompileOne = function(memory, address)
{
	this.memory = memory;
	this.address = address;
	
	var op = this.nextInstruction();
	var code = OldRecompiler.functionPrelude
		+ "var pc = 0x" + OldRecompiler.formatHex(address) + " + 4;\n"
		+ "do {\n"
...
OldRecompiler.prototype.addInjector = function(injector)
{
	return this.injectors.push(injector);
}

OldRecompiler.prototype._injectBefore = function(address, opcode, isDelaySlot)
{
	return this._injectCallback("injectBeforeInstruction", address, opcode, isDelaySlot);
}

OldRecompiler.prototype._injectAfter = function(address, opcode, isDelaySlot)
{
	return this._injectCallback("injectAfterInstruction", address, opcode, isDelaySlot);
}

OldRecompiler.prototype._injectAtLabel = function(address)
{
	return this._injectCallback("injectBeforeLabel", address);
}

OldRecompiler.prototype._injectCallback = function(fn)
{
	var injected = "";
...
OldRecompiler.unsign = function(x)
{
	var lastBit = x & 1;
	return (x >>> 1) * 2 + lastBit;
}

OldRecompiler.formatHex = function(address, length)
{
	if (length === undefined) length = 8;
	var output = OldRecompiler.unsign(address).toString(16);
...
;(function()
{
	...
	{
		OldRecompiler.prototype[inst] = func;
	}
	...
	function hex(x)
	{
		...
		return "0x" + OldRecompiler.formatHex(x);
	}
	...
	function branch(condition, offset)
	{
		...
		var targetAddress = OldRecompiler.unsign(opAddress + (signExt(offset, 16) << 2));
	...
	impl("j", function(i) {
		...
		var jumpAddress = OldRecompiler.unsign((opAddress & 0xF0000000) | (i << 2));
